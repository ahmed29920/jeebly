        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('admin.offers.update', $offer->id) }}" method="POST" id="offerForm">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="type" id="offer_type" class="form-control" required>
                                ...
                                <option value="product" {{ old('type', $offer->type) == 'product' ? 'selected' : '' }}>{{ __('Product') }}</option>
                                <option value="category" {{ old('type', $offer->type) == 'category' ? 'selected' : '' }}>{{ __('Category') }}</option>
                                <option value="cart" {{ old('type', $offer->type) == 'cart' ? 'selected' : '' }}>{{ __('Cart') }}</option>
                                <option value="shipping" {{ old('type', $offer->type) == 'shipping' ? 'selected' : '' }}>{{ __('Shipping') }}</option>
                            </select>
                            @error('type')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="discount_type" id="discount_type" class="form-control" required>
                                ...
                                <option value="fixed" {{ old('discount_type', $offer->discount_type) == 'fixed' ? 'selected' : '' }}>{{ __('Fixed Amount') }}</option>
                                <option value="percent" {{ old('discount_type', $offer->discount_type) == 'percent' ? 'selected' : '' }}>{{ __('Percentage') }}</option>
                                <option value="free_shipping" {{ old('discount_type', $offer->discount_type) == 'free_shipping' ? 'selected' : '' }}>{{ __('Free Shipping') }}</option>
                                <option value="bogo" {{ old('discount_type', $offer->discount_type) == 'bogo' ? 'selected' : '' }}>{{ __('Buy One Get One') }}</option>
                            </select>
                            @error('discount_type')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                            ...
                            <div class="col-md-12 mb-3">
                                        ...
                                        @if(isset($products))
                                            @foreach($products as $product)
                                                <option value="{{ $product->id }}"
                                                    {{ old('condition.product_id', $offer->getConditionValue('product_id')) == $product->id ? 'selected' : '' }}>
                                        ...
                                        @if(isset($categories))
                                            @foreach($categories as $category)
                                                <option value="{{ $category->id }}"
                                                    {{ old('condition.category_id', $offer->getConditionValue('category_id')) == $category->id ? 'selected' : '' }}>
                                ...
                                <div id="condition_cart" style="display: none;">
                                    ...
                                    <input type="number" step="0.01" min="0" name="condition[min_cart_amount]"
                                        class="form-control"
                                        value="{{ old('condition.min_cart_amount', $offer->getConditionValue('min_cart_amount', 0)) }}"
                    ...
                    <button class="btn btn-primary mt-3" type="submit">{{ __('Update Offer') }}</button>
