            ...
            <div class="col-lg-11 mx-auto">
                <div class="card border-0 shadow-sm mb-4 rounded-4 bg-white" style="background: #ffffff !important; border-color: transparent !important;">
                    <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center py-3" style="background: #ffffff !important;">
                        ...
                        <a href="{{ route('admin.deliveries.index') }}" class="btn btn-sm btn-secondary">
                            <i class="fas fa-arrow-left me-1"></i>{{ __('Back') }}
                        </a>
                    </div>
                    <div class="card-body" style="background: #ffffff !important;">
                        <form action="{{ route('admin.deliveries.update', $delivery->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="branch_id"
                                                class="form-select shadow-sm @error('branch_id') is-invalid @enderror">
                                            <option value="">{{ __('Select Branch') }}</option>
                                            @foreach ($branches as $branch)
                                                <option value="{{ $branch->id }}"
                                                    {{ old('branch_id', $delivery->branch_id) == $branch->id ? 'selected' : '' }}>
                                                    {{ $branch->name }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="row g-3">
                                    <div class="col-md-12">
                                        <label class="form-label fw-semibold">
                                            <i class="fas fa-map-marked-alt text-primary me-2"></i>{{ __('Assign Zones') }}
                                        </label>
                                        @php
                                            $selectedZones = old('zone_ids', $assignedZoneIds ?? []);
                                        @endphp
                                        <select name="zone_ids[]" class="form-select shadow-sm @error('zone_ids') is-invalid @enderror @error('zone_ids.*') is-invalid @enderror" multiple>
                                            @foreach(($zones ?? collect()) as $zone)
                                                <option value="{{ $zone->id }}"
                                                    {{ in_array($zone->id, $selectedZones, true) ? 'selected' : '' }}>
                                                    #{{ $zone->id }} - {{ $zone->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <small class="text-muted d-block mt-1">{{ __('Select the zones this delivery can cover.') }}</small>
                            ...
                            <div class="mt-4 pt-3 border-top">
                                ...
                                <a href="{{ route('admin.deliveries.index') }}" class="btn btn-secondary shadow-sm px-4">
