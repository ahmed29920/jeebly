...
<script>
    ...
    // Products Quantities Sold Over Time
    const productsSoldData = @json($productsSoldOverTime);
    const productNames = [...new Set(productsSoldData.map(item => item.name))];
    const months = [...new Set(productsSoldData.map(item => item.month))];
    ...
    const datasets = productNames.map((name, index) => {
        ...
        return {
            ...
            data: months.map(month => {
                const record = productsSoldData.find(item => item.name === name && item.month === month);
                return record ? record.total_sold : 0;
            }),
            borderColor: color,
            backgroundColor: color.replace('50%', '70%'),
            tension: 0.3
        };
    });
